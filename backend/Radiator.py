class Radiator:
    def getRadiatorData(self, T_in, T_room, V_flow):
        ...
        # Flow properties
        v_water = (V_flow / 1000) / A_pipe  # m/s, velocity of water through radiator
        Re_d = (rho_water * v_water * D_pipe) / dyn_visc  # Reynolds number of pipe

        # Laminar
        if Re_d <= 3000:
            Nu_D = 3.66  # Nusselt number using laminar correlation
        # Gnielinski Correlation
        elif 3000 <= Re_d < 5 * 10 ** 6 and 0.5 <= Pr <= 2000:
            f = (0.79 * np.log(Re_d) - 1.64) ** -2  # f value for Gnielinski correlation
            Nu_D = ((f / 8) * (Re_d - 1000) * Pr) / (1 + (12.7 * (f / 8) ** 0.5) * (
                        Pr ** (2 / 3) - 1))  # Nusselt number using Gnielinski correlation
        # Dittus-Boelter Correlation
        elif Re_d >= 10000 and 0.6 <= Pr <= 160:
            Nu_D = 0.023 * Re_d ** (4 / 5) * Pr ** 0.3  # Nusselt number using Dittus-Boelter correlation
        h_avg = Nu_D * k_water / D_pipe  # W/m^2K, convection heat transfer coefficient of water through pipe at 80C
        mass_flow_rate = v_water * A_pipe * rho_water  # kg/s, mass flow rate of water
        ...
        Ra_L = (g * beta * abs(T_surface - T_room) * height_radiator ** 3) / (kin_visc_air * alpha_air)
        Nu_L = (0.825 + (0.387 * Ra_L ** (1 / 6)) / (1 + (0.492 / Pr_air) ** (9 / 16)) ** (8 / 27)) ** 2
        ...
        R_water = 1 / (h_avg * np.pi * D_pipe * L_pipe)  # K/W, R value of water through pipe
        R_pipe = np.log((D_pipe / 2 + t_pipe) / (D_pipe / 2)) / (2 * np.pi * k_pipe * L_pipe)  # K/W, R value of pipe
        R_air = 1 / (h_air * np.pi * (D_pipe + 2 * t_pipe) * L_pipe)  # K/W, R value of water through pipe
        R_total = R_water + R_pipe + R_air  # m^2K/W, total R value of radiator

        def cal_T_out(T_room, T_in):
            return T_room - np.exp(-1 / (mass_flow_rate * c_p_water * R_total)) * (T_room - T_in)
        ...
        Q_out = mass_flow_rate * c_p_water * (T_in - T_out)
